...
if(count($content_info)==0){
	message(array('text_en'=>$language['content_is_not_exist'],'link'=>''));
}
if($content_info['state']==0){
	message(array('text_en'=>$language['content_is_lock'],'link'=>''));
}
...
$smarty->display('en_content.html');
...
function prev_content($content_id,$channel_id){
	$row=$GLOBALS['db']->getone("select content_id,content_title,content_title_en,content_url from ".$GLOBALS['db_prefix']."content WHERE content_id>".$content_id." and content_state!=0 and channel_id='".$channel_id."' ORDER BY content_id ASC LIMIT 0,1");
	if($row){
		return array(
			'id'=>$row['content_id'],
			'title'=>$row['content_title'],
			'title_en'=>$row['content_title_en'],
...
function next_content($content_id,$channel_id){
	$row=$GLOBALS['db']->getone("select content_id,content_title,content_title_en,content_url from ".$GLOBALS['db_prefix']."content WHERE content_id<".$content_id." and content_state!=0 and channel_id='".$channel_id."' ORDER BY content_id DESC LIMIT 0,1");
	if($row){
		return array(
			'id'=>$row['content_id'],
			'title'=>$row['content_title'],
			'title'=>$row['content_title_en'],
