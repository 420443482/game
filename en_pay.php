...
if($action=='pay_ok'){
	...
	if($game_id==0||$server_id==0){
		message(array('text'=>'Please choose the recharge game and the server!','link'=>''));
	}
	if($game_user==''||$game_user!=$game_user2){
		message(array('text'=>'Recharge account input error!','link'=>''));
	}
	...
	message(array('text'=>'Start to enter the payment.。。。','link'=>'','jump'=>'0'));
}
