...
var itemEmots={'default':{name:'默认',width:25,height:18,line:7,list:{'emot1':'惊恐','emot2':'飞吻','emot3':'迷糊','emot4':'酷毙了','emot5':'大哭','emot6':'吐舌头','emot7':'闭嘴','emot8':'得意','emot9':'流口水','emot10':'偷笑','emot11':'发火','emot12':'脸红','emot13':'无语','emot14':'汗死','emot15':'欢喜','emot16':'激动','emot17':'感动','emot18':'伤心','emot19':'还行','emot20':'目瞪口呆','emot21':'大笑'}}};
...
$.xheditor=function(textarea,options)
{
	...
	editorRoot=getLocalUrl(editorRoot,'rel').replace('scripts/','');
	//基本控件名
	...
	var headHTML='',bodyClass='',skinPath=editorRoot,arrEmots=itemEmots,urlType=settings.urlType,urlBase=settings.urlBase,emotPath=editorRoot+'images/',emotPath=emotPath?emotPath:editorRoot+'images/',selEmotGroup='';
	...
	{
		//加载样式表
		if($('#'+idCSS).length===0)$('head').append('<link id="'+idCSS+'" rel="stylesheet" type="text/css" href="styles/ui.css" />');
		//初始化编辑器
		...
		headHTML='<meta content="text/html; charset=UTF-8" http-equiv="Content-Type"/><link rel="stylesheet" href="styles/iframe.css"/>';
	...
	this.showEmot=function(group)
	{
		...
		var sEmotPath=emotPath,n=0,arrList=[],jList='';
		...
		_this.pasteHTML('<img src="images/'+jA.attr('emot').split(',')[1]+'.gif" alt="'+jA.attr('title')+'">');
		_this.hidePanel();
		return false;}).mousedown(function(){return false;});
		jEmot.append(jList);
	...
	this.startUpload=function(fromFiles,toUrl,limitExt,onUploadComplete)
	{
		...
		var upload,fileList,filename,jUploadTip=$('<div style="padding:22px 0;text-align:center;line-height:30px;">文件上传中，请稍候……<br /></div>'),sLoading='<img src="images/loading.gif">';
