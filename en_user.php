...
if($action=='register_ok'){
	...
	*/
	...
	//注册成功
	message(array('text'=>$language['join_member_success'],'link'=>'en_user.php'));
}
//用户退出
if($action=='logout'){
	check_request();
	...
	*/
	...
	//退出成功
	message(array('text'=>$language['logout_success'],'link'=>'en_index.php'));
}
...
if($action=='save'){
	...
	//修改成功
	message(array('text'=>'修改成功！','link'=>'en_user.php'));
}
...
if($action=='paylog'){
	...
	$smarty->display('en_pay_log.html');
	exit;
}
//新手卡
if($action=='card'){
	...
	$smarty->assign('pagebar',$pagebar);
	$smarty->display('en_card_log.html');
	exit;
}
//修改密码
if($action=='setpass'){
	$smarty->assign('mode','setpass');
	$smarty->display('en_user_info.html');
	exit;
}
//修改密保
if($action=='setsec'){
