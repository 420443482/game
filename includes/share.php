...
function get_content_info($content_id){
	...
	$array['title_en']			=$row['content_title_en'];
	...
	$array['text_en']		=$row['content_text_en'];
...
function login_passed(){
	if(!check_login()){
		message(array('text'=>'您必須登錄才能進行此操作；如果您還沒有通行證帳號，請先註冊。','link'=>'login.php'));
	}
}
function en_login_passed(){
	if(!check_login()){
		message(array('text'=>'You must log in to do this; if you do not have a passport account, please register first.。','link'=>'en_login.php'));
	}
}
function get_login(){
	$array=array();
...
function content_array_list($channel_id, $top){
    ...
    foreach ($res AS $row){
		...
		$array[$row['content_id']]['content_title_en']=truncate($row['content_title_en'],30);
