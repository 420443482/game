...
function get_diy_content($channel_id,$category_id=0,$limit=20){
	...
	if(count($res)>0){
		foreach($res as $row){
			...
			$array[$row['content_id']]['title_en']=truncate($row['content_title_en'],$channel_info['index_truncate']);
			$array[$row['content_id']]['text']=truncate(strip_tags($row['content_text']),40);
			$array[$row['content_id']]['text_en']=truncate(strip_tags($row['content_text_en']),40);
...
function get_hot_content($channel_id=0){
    ...
    foreach ($res AS $row){
		...
		$array[$row['content_id']]['title_en']=truncate($row['content_title_en'],$GLOBALS['config']['content_hot_title_size']);
		$array[$row['content_id']]['thumb']=$row['content_thumb'];
		if(empty($row['content_url'])){
			$array[$row['content_id']]['url']=create_uri('content',array('id'=>$row['content_id']));
			$array[$row['content_id']]['target']=false;
...
function pagebar($page_name,$page_parameters='',$page_current,$page_size,$count){
	...
	if($page_current!=1){
		if($rewrite=='yes'){
			...
			$html.="<a href='".create_uri($page_name,$rewrite_parameters)."'>&laquo;</a>";
		}else{
			$html.="<a href='".$page_name.".php?".$page_parameters."page=1'>&laquo;</a>";
		}
	}
	for($i=$page_start;$i<=$page_end;$i++){
		if($i==$page_current){
			...
			if($rewrite=='yes'){
				...
				$html.="<a href='".create_uri($page_name,$rewrite_parameters)."'>".$i."</a>";
			}else{
				$html.="<a href='".$page_name.".php?".$page_parameters."page=".$i."'>".$i."</a>";
			}
		}
	}
	if($page_current!=$page_count){
		if($rewrite=='yes'){
			...
			$html.="<a href='".create_uri($page_name,$rewrite_parameters)."'>&raquo;</a>";
		}else{
			$html.="<a href='".$page_name.".php?".$page_parameters."page=".$page_count."'>&raquo;</a>";
		}
	}
